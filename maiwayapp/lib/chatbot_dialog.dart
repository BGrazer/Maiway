...
final String _chatBackendUrl = 'http://192.168.254.105:5001/chat';
final String _dynamicSuggestionsUrl = 'http://192.168.254.105:5001/dynamic_suggestions'; 
  ...
  Future<void> _onInputChanged(String query) async {
    ...
    try {
      final response = await http.get(Uri.parse('$_dynamicSuggestionsUrl?query=${Uri.encodeComponent(query)}'));

      if (response.statusCode == 200) {
        final Map<String, dynamic> responseData = jsonDecode(response.body);
        final List<dynamic> fetchedSuggestions = responseData['suggestions'] ?? [];
              ...
              children: [
                Container(
                  ...
                  child: Row(
                    children: [
                      IconButton(
                        icon: const Icon(FontAwesomeIcons.chevronLeft, size: 20, color: Colors.white),
                        onPressed: () {
                ...
                if (_dynamicSuggestions.isNotEmpty)
                  Container(
                    ...
                    child: Wrap(
                      spacing: 8.0,
                      runSpacing: 8.0,
                      children: _dynamicSuggestions.map((text) => _buildSuggestionChip(text)).toList(),
                    ),
                  ),
            ...
            return AnimatedBuilder(
              ...
              builder: (context, child) {
                final double opacity = (index == 0
                    ...
                    2;
                return Opacity(
                  opacity: (opacity > 1.0 ? 2.0 - opacity : opacity).clamp(0.2, 1.0),
                  child: const Text(
                    '.',
                    style: TextStyle(
                      fontSize: 20,
